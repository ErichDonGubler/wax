...
pub type JoinResult<L, R> = Result<<(L, R) as Join>::Output, <(L, R) as Join>::Error>;
...
impl<'t> GlobError<'t> {
    ...
    pub fn into_owned(self) -> GlobError<'static> {
        match self {
            ...
            GlobError::Path(error) => GlobError::Path(error),
...
impl From<PathError> for GlobError<'static> {
    fn from(error: PathError) -> Self {
        GlobError::Path(error)
    }
}
...
impl<'t> TryFrom<Tokenized<'t>> for Glob<'t> {
    type Error = GlobError<'t>;

    // TODO: Unify this and other `TryFrom` implementations with `Glob::new` and
    //       `parse_and_check`.
    fn try_from(tokenized: Tokenized<'t>) -> Result<Self, Self::Error> {
...
pub fn join<L, R>(left: L, right: R) -> JoinResult<L, R>
where
    (L, R): Join,
{
    (left, right).join()
